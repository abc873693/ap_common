...
提供校園最新消息列表。v3 將消息項目提取為獨立的 `NotificationItem` 元件，並改用 Material Design 3 的 `colorScheme` 取得主題色彩。
...
自 v3 起從 `NotificationListView` 內部提取為獨立元件，顯示單一校園最新消息項目（包含標題、日期、單位）。

| 參數名稱                         | 型態                             | 描述                             |
| ------------------------------- | ------------------------------- | ------------------------------- |
| notification | `Notifications` | 校園最新消息資料物件 |