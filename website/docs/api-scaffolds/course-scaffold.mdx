...
提供顯示課表(Course Table)骨架，另將課表 `CourseContent` 與課程列表 `CourseList` 另拆成兩個 Widget。自 v2 版本起，大幅更新了外觀與操作體驗，並與 `SemesterPicker` 有更好的整合。v3 全面改用 Material Design 3 的 `colorScheme` 取得主題色彩。
 - 支援響應式介面
