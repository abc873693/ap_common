...
:::info 實作類別
目前可使用的實作類別有 `ApNotificationUtil`

必需透過套件 [`ap_common_flutter_platform`](https://pub.dev/packages/ap_common_flutter_platform)，為套件 [`flutter_local_notification`](https://pub.dev/packages/flutter_local_notifications) 的封裝細節可參考原專案，

使用時必須先完成 [相關設定](../installation/local-notification)
:::

本地通知顯示，通常是作業系統的彈跳視窗或通知

需透過 `registerApCommonFlutter()` 指定成實例
