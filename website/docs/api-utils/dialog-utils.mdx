...
以簡易方法開啟的 [一般對話框](docs/api-widgets/default-dialog)
...
版本內容([`VersionInfo`](docs/api-models/version-info) 中的 `content`)，若有設定 `githubRepositoryName` 與 `githubBranchName`，內容則會從 GitHub 專案中讀取 `changelog.json`
...
| ------------------------------- | ------------------------------- | ------------------------------- |
| context | BuildContext | Flutter Widget context |
| versionInfo | [`VersionInfo`](docs/api-models/version-info) | 版本資訊 |
| appName | String |  名稱 |
